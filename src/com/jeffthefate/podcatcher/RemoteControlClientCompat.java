...
public class RemoteControlClientCompat {
    ...
    public static int FLAG_KEY_MEDIA_NEXT = 128;

    static {
        try {
            ClassLoader classLoader = 
                RemoteControlClientCompat.class.getClassLoader();
            sRemoteControlClientClass = getActualRemoteControlClientClass(
                    classLoader);
            ...
            for (Field field : RemoteControlClientCompat.class.getFields()) {
                try {
                    Field realField = sRemoteControlClientClass.getField(
                            field.getName());
                    Object realValue = realField.get(null);
                    field.set(null, realValue);
                } catch (NoSuchFieldException e) {
                    Log.e(Constants.LOG_TAG, "Could not get real field: " +
                            field.getName());
                } catch (IllegalArgumentException e) {
                    Log.e(Constants.LOG_TAG, "Error trying to pull field " +
                            "value for: " + field.getName() + " " +
                            e.getMessage());
            ...
            sRCCEditMetadataMethod = sRemoteControlClientClass.getMethod(
                    Constants.EDIT_METADATA, boolean.class);
            sRCCSetPlayStateMethod = sRemoteControlClientClass.getMethod(
                    Constants.SET_PLAYBACK_STATE, int.class);
            sRCCSetTransportControlFlags = sRemoteControlClientClass.getMethod(
                    Constants.SET_TRANSPORT_CONTROL_FLAGS, int.class);
    ...
    public static Class getActualRemoteControlClientClass(
        ...
        return classLoader.loadClass(Constants.REMOTE_CONTROL_CLIENT);
    }
    ...
    public RemoteControlClientCompat(PendingIntent pendingIntent) {
        ...
        try {
            mActualRemoteControlClient =
                    sRemoteControlClientClass.getConstructor(
        ...
        try {
            mActualRemoteControlClient = 
                sRemoteControlClientClass.getConstructor(PendingIntent.class,
                        Looper.class).newInstance(pendingIntent, looper);
        } catch (Exception e) {
            Log.e(Constants.LOG_TAG, "Error creating new instance of " + 
                    sRemoteControlClientClass.getName(), e);
        }
    }
    ...
    public class MetadataEditorCompat {
        ...
        private MetadataEditorCompat(Object actualMetadataEditor) {
            if (sHasRemoteControlAPIs && actualMetadataEditor == null) {
                throw new IllegalArgumentException("Remote Control API's " +
                        "exist, should not be given a null MetadataEditor");
            }
            if (sHasRemoteControlAPIs) {
                Class metadataEditorClass = actualMetadataEditor.getClass();

                try {
                    mPutStringMethod = metadataEditorClass.getMethod(
                            Constants.PUT_STRING, int.class, String.class);
                    mPutBitmapMethod = metadataEditorClass.getMethod(
                            Constants.PUT_BITMAP, int.class, Bitmap.class);
                    mPutLongMethod = metadataEditorClass.getMethod(
                            Constants.PUT_LONG, int.class, long.class);
                    mClearMethod = metadataEditorClass.getMethod(
                            Constants.CLEAR, new Class[]{});
                    mApplyMethod = metadataEditorClass.getMethod(
                            Constants.APPLY, new Class[]{});
                } catch (Exception e) {
    ...
    public MetadataEditorCompat editMetadata(boolean startEmpty) {
        ...
        if (sHasRemoteControlAPIs) {
            try {
                metadataEditor = sRCCEditMetadataMethod.invoke(
                        mActualRemoteControlClient, startEmpty);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else {
    ...
    public void setPlaybackState(int state) {
        if (sHasRemoteControlAPIs) {
            try {
                sRCCSetPlayStateMethod.invoke(mActualRemoteControlClient,
                        state);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
